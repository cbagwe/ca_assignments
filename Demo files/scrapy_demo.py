...
class DebateBaseSpider(scrapy.Spider):
    name = "Debate"
    start_urls = ['https://myanimelist.net/Debate.php']

    def parse(self, response):
        xp = "//div[@id='horiznav_nav']//li/a/@href"
        return (Request(url, callback=self.parse_Debate_list_page) for url in response.xpath(xp).extract())
    
    def parse_Debate_list_page(self, response):
